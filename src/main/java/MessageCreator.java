...
public class MessageCreator {
    ...
    private byte[] createPackage(byte[] message){
        byte[] res = new byte[18+message.length];
        res[0] = 0x13;
        res[1] = B_SRC;
        ...
        convertToBigEndianShort(makeCRC16(tmp), (byte)2, 14, 16, res);
        for(int i = 16; i<message.length+16; i++){
            res[i] = message[i-16];
        }
        tmp = new byte[message.length];
        for(int i = 16; i<16+message.length; i++) {
            tmp[i-16] = res[i];
        }
        convertToBigEndianShort(makeCRC16(tmp), (byte)2, 16+message.length, 18+message.length, res);
        return res;
    }
    ...
    private short makeCRC16(byte[] packet){
        int[] table = {
